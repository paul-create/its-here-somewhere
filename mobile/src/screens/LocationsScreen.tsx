...
export function LocationsScreen({ navigation }: any) {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Error loading locations';
      setError(errorMsg);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {locations.length === 0 ? (
        <View style={[styles.container, styles.centered]}>
          <MaterialCommunityIcons name="map-outline" size={48} color="#cccccc" />
          <Text style={styles.emptyText}>No locations yet</Text>
          <Text style={styles.emptySubtext}>Tap + to create your first location</Text>
        </View>
      ) : (
        <FlatList
          data={locations}
